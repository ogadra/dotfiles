...
見出しの言い換えと、見出しの語から読み手が当然そうなると分かることが入る。コミットメッセージの件名と本文がこの形になりやすい。
...
Switch the retry interval to exponential backoff

Each failure makes the next wait longer.
```
...
本文の各文を、見出しだけ読んだ読み手が言い当てられるか確かめる。
...
その見出しに置く中身を持っているか確かめる。

- 持っている
    - 見出しに書いたことを本文に置く
- 持っていない
    - 節ごと消す
    - `目的` `背景` `課題` の下にやることの言い換えがあるものがこれにあたる
- 見出しの語とずれた中身を持っている
    - 見出しのほうを中身に合わせて変える

知らないことは書かない。埋めた理由を、読み手は書き手の判断として読む。

### しないことの記述

やらなかったこと、読み手に取らせない選択肢を本文に残す形。読み手がまだ聞いていないことに先回りして答えている。
...
手順書と設定の説明で、知っている事実を置ける場所に置いてしまう。読み手に何をさせたい一文なのかを決めていない。
...
1. 見出しをすべて抜き出す
    - 名詞句になっていないものを集める
2. 直下の本文がその見出しの語に答えているか、見出しごとに確かめる
    - 優先して見る見出し
        - `目的`
        - `背景`
        - `課題`
        - `概要`
3. 本文の文ごとに、見出しだけ読んだ読み手が言い当てられるか確かめる
    - 見出しが動詞で始まる場合
        - その動作に伴う結果を本文に書いていないか確かめる
4. しないことの記述を集める
    - 探す見出し
        - `対象外`
        - `スコープ外`
