...
指摘は、文書に残す内容にだけ出す。書き手が何を試し、何をどう確かめたかを書いた箇所は、文書構成のレビュアーが消す。

## 検出対象

### 平坦な箇条書き

理由や補足を持つ項目が、第1階層に同じ深さ、同じ長さで並ぶ。書き手が、項目名とその説明を `**項目名**: 説明` の形で1行に押し込んでいる。
...
箇条書きの1項目に、読点やカンマで複数の語が並ぶ。読み手は、その行を読んだだけでは並列なのか補足なのかを決められない。
