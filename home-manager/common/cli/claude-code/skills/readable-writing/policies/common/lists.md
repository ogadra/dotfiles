...
指摘は、文書に残す内容にだけ出す。書き手が何を試し、何をどう確かめたかを書いた箇所は、箇条書きに組み替えない。消す指摘は文書構成のレビュアーが出す。

## 検出対象

### 平坦な箇条書き

理由や補足を持つ項目が、第1階層に同じ深さ、同じ長さで並ぶ。項目名とその説明が `**項目名**: 説明` の形で1行に押し込まれている。

#### AI版

```
- **長さのムラ**: 段落と文の長さをバラバラにする。1行で終わる段落と10行続く段落を混ぜる。
- **密度のムラ**: 興味のある話題には固有名詞・数値・事実を詰め、興味のないところは短く書く。
...
箇条書きの1項目に、読点やカンマで複数の語が並ぶ。読み手は、その行を読んだだけでは並列なのか補足なのかを決められない。1つだけ引用したいときにも切り出せない。
...
1項目に `XならY` が収まっている。条件と処置は別のことなので、読み手は1行の中で2つを分解してから、他の項目の条件と突き合わせることになる。分岐が3つ並ぶと、どこまでが条件でどこからが処置なのか目で追えなくなる。
